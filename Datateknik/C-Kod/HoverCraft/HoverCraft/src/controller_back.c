...
void start_controller_back(void *p)
{
	...
	while(1)
	{
		...
		back_left_inductor = ADC->ADC_CDR[2]; // Retrieve latest vaule from back left inductor
		int back_right_inductor;
		back_right_inductor = ADC->ADC_CDR[1]; // Retrieve latest value from back right inductor
